"""
Entry point and agent definition for training MAPPO-PS. Code largely based on CleanRL implementation of PPO.

Usage:
    # default: imprl/agents/configs/MAPPO_PS.yaml
    python imprl/agents/MAPPO_PS.py

    # override environment setting and seed
    python imprl/agents/MAPPO_PS.py ENV_CONFIG.env_setting=hard-2-of-4_infinite SEED=7
...
class MultiAgentProximalPolicyOptimizationParameterSharing:
    name = "MAPPO-PS"
    full_name = "Multi-Agent Proximal Policy Optimization"
    ...
    def __init__(self, env, config=None, device=None, num_envs=4):
        ...
        critic_input = np.prod(envs.single_observation_space.shape)
    ...
    def select_action(self, x, training=False):
        ...
        MAPPO uses the plain per-agent percept and appends the agent identity
...
@hydra.main(config_path="configs", config_name="MAPPO_PS", version_base=None)
def main(cfg: DictConfig):
    ...
    learning_agent = MultiAgentProximalPolicyOptimizationParameterSharing(
    ...
    for iteration in range(1, cfg.num_iterations + 1):
        ...
        for _ in range(cfg.update_epochs):
            ...
            for start in range(0, cfg.batch_size, cfg.minibatch_size):
                ...
                pg_loss = torch.sum(torch.max(pg_loss1, pg_loss2), dim=1).mean()
