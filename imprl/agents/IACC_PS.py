"""
Single-file implementation of IACC-PS.

IACC-PS: Independent Actor Centralized Critic with Parameter Sharing.
...
- a centralized critic value over the full system belief/state.
...
class IndependentActorCentralisedCriticParameterSharing:
    name = "IACC-PS"  # display names used by experiment scripts/loggers.
    ...
    def __init__(self, env, config, device):
        """Initialize shared actor, centralized critic, replay buffer, and counters."""
        assert (
            env.__class__.__name__ == "MultiAgentWrapper"
        ), "IACC-PS only supports multi-agent environments"
        ...
        n_inputs_actor = local_obs.shape[-1]
