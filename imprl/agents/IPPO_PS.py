"""
Entry point and agent definition for training IPPO-PS. Code largely based on CleanRL implementation of PPO.

Usage:
    # default: imprl/agents/configs/IPPO_PS.yaml
    python imprl/agents/IPPO_PS.py

    # override environment setting and seed
    python imprl/agents/IPPO_PS.py ENV_CONFIG.env_setting=hard-2-of-4_infinite SEED=7
...
class WrapperForVecEnv:
    """Adapt the multi-agent env to a fixed numeric Box space for rollouts.
    This is required because we use Gym's `SyncVectorEnv` for parallel rollouts.

    The trainer stores fixed-shape arrays, so this wrapper converts each raw
    env percept to `multiagent_percept(...)` before it enters the rollout
    buffers.
    """
    ...
    def __init__(self, env):
        ...
        obs, _ = env.reset()
        obs = env.multiagent_percept(obs)
    ...
    def reset(self):
        ...
        return self.env.multiagent_percept(obs), info

    def step(self, actions):
        obs, reward, terminated, truncated, info = self.env.step(actions)
        return (
            self.env.multiagent_percept(obs),
            reward,
            terminated,
            truncated,
            info,
...
class IndependentProximalPolicyOptimizationParameterSharing:
    name = "IPPO-PS"  # display names used by experiment scripts/loggers.
    full_name = "Independent Proximal Policy Optimization"
    ...
    def __init__(self, env, config=None, device=None, num_envs=4):
        ...
        self.n_agents = envs.single_observation_space.shape[0]
        actor_input = envs.single_observation_space.shape[-1] + self.n_agents
    ...
    def add_agent_ids(self, x):
        x = torch.as_tensor(x, dtype=torch.float32, device=self.device)
        eye = torch.eye(self.n_agents, dtype=x.dtype, device=x.device)
        if x.dim() == 2:
            return torch.cat([eye, x], dim=-1)
        if x.dim() == 3:
            batched_eye = eye.unsqueeze(0).expand(x.shape[0], -1, -1)
            return torch.cat([batched_eye, x], dim=-1)
        raise ValueError(f"Expected 2D or 3D input, got shape {tuple(x.shape)}")

    def get_value(self, x):
        ma_x = self.add_agent_ids(x)
        return self.critic(ma_x)

    def get_action_and_value(self, x, action=None):
        ma_x = self.add_agent_ids(x)
        logits = self.actor(ma_x)
        probs = Categorical(logits=logits)
        if action is None:
            action = probs.sample()
        return action, probs.log_prob(action), probs.entropy(), self.critic(ma_x)

    def select_action(self, x, training=False):
        """Select one action per agent for inference.

        IPPO uses the plain per-agent percept and appends the agent identity
        matrix internally before running the shared actor.
        """
        x = self.base_env.multiagent_percept(x)
        ma_x = self.add_agent_ids(x)
        logits = self.actor(ma_x)
        probs = Categorical(logits=logits)
...
@hydra.main(config_path="configs", config_name="IPPO_PS", version_base=None)
def main(cfg: DictConfig):
    ...
    learning_agent = IndependentProximalPolicyOptimizationParameterSharing(
        env, alg_config, device=device
    )
    ...
    actions = torch.zeros(
        ...
        + (learning_agent.n_agents,)
    ).to(device)
    logprobs = torch.zeros((cfg.num_steps, cfg.num_envs, learning_agent.n_agents)).to(
        device
    )
    rewards = torch.zeros((cfg.num_steps, cfg.num_envs)).to(device)
    dones = torch.zeros((cfg.num_steps, cfg.num_envs)).to(device)
    values = torch.zeros((cfg.num_steps, learning_agent.n_agents, cfg.num_envs)).to(
        device
    )
    ...
    for iteration in range(1, cfg.num_iterations + 1):
        ...
        for step in range(0, cfg.num_steps):
            ...
            with torch.no_grad():
                ...
                values[step] = value.squeeze().T
            actions[step] = action
            logprobs[step] = logprob

            next_obs, reward, terminations, truncations, _ = envs.step(
                action.cpu().numpy()
            )
        ...
        with torch.no_grad():
            # transpose: (num_envs, num_agents) --> (num_agents, num_envs)
            next_value = learning_agent.get_value(next_obs).squeeze().T
        ...
        b_logprobs = logprobs.reshape(-1, learning_agent.n_agents)
        b_actions = actions.reshape(
            (-1,) + envs.single_action_space.shape + (learning_agent.n_agents,)
        )
        # reshape values and advantages:
        # (S, M, N) -> (S, N, M) -> (S*N, M)
        b_advantages = advantages.permute(0, 2, 1).reshape(-1, learning_agent.n_agents)
        b_returns = returns.permute(0, 2, 1).reshape(-1, learning_agent.n_agents)
        b_values = values.permute(0, 2, 1).reshape(-1, learning_agent.n_agents)
        ...
        for _ in range(cfg.update_epochs):
            ...
            for start in range(0, cfg.batch_size, cfg.minibatch_size):
                ...
                with torch.no_grad():
                    old_approx_kls = (-logratio).mean(dim=0)
                    approx_kls = ((ratio - 1) - logratio).mean(dim=0)

                mb_advantages = b_advantages[mb_inds]
                if cfg.norm_adv:
                    mb_advantages = (mb_advantages - mb_advantages.mean(dim=0)) / (
                ...
                pg_loss = torch.max(pg_loss1, pg_loss2).sum(dim=1).mean()
                ...
                torch.nn.utils.clip_grad_norm_(
                    learning_agent.parameters(), cfg.max_grad_norm
                )
                optimizer.step()
        ...
        for i in range(learning_agent.n_agents):
            training_log[f"losses/old_approx_kl_{i+1}"] = old_approx_kls[i].item()
            training_log[f"losses/approx_kl_{i+1}"] = approx_kls[i].item()
