"""
Singled-file implementation of DCMAC.

DCMAC: Deep Centralized Multiagent Actor Critic.
This module implements an off-policy centralized actor-critic for cooperative
multi-agent control.
...
Architecture:
    ...
    Actor (centralized → joint action):
    [b1|...|bN] -> [ACTOR] -> (A1 x ... x AN)
"""
...
class DeepCentralisedMultiAgentActorCritic:
    name = "DCMAC"  # display names used by experiment scripts/loggers.
    full_name = "Deep Centralised Multi-Agent Actor-Critic"
    ...
    def __init__(self, env, config, device):
        ...
        ), "DCMAC only supports multi-agent environments"
        ...
        actor_arch += [self.n_agents * self.n_agent_actions]
        critic_arch = [n_inputs] + self.cfg["CRITIC_CONFIG"]["hidden_layers"] + [1]

        # ---------- Actor / critic modules ----------
        self.actor = ActorNetwork(
            actor_arch,
            reshaping=(self.n_agents, self.n_agent_actions),
    ...
    def select_action(self, observation, training):
        ...
        # Centralized perception for all components.
        ...
        # Joint action sampled from component-wise categorical heads.
    ...
    def compute_loss(self, *args):
        ...
        joint_log_probs = action_dists.log_prob(t_actions).sum(dim=-1)
